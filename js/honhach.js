...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  function renderHonhachGrid(list) {
    ...
    honhachGrid.innerHTML = list.map(item => {
      ...
      let rolesDisplay = i18n.getLanguage() === 'en' ? 'Heavy Attack / Agility' : 'Cường Công / Mẫn Công';
      if (rolesList.includes('phu_tro') || rolesList.includes('khong_che') || rolesList.includes('phong_thu')) {
        rolesDisplay = i18n.getLanguage() === 'en' ? 'Support / Control / Defense' : 'Phụ Trợ / Khống Chế / Phòng Thủ';
      }
        ...
        set2Html = `
          <div class="set-box">
            <div class="set-box-title">
              <span>${i18n.t('honhach_set2_title')}</span>
      ...
      if (item.set4) {
        ...
        const extra24Html = item.set4.extra24 ? `
          <div style="margin-top:0.5rem; background:rgba(245,158,11,0.1); border:1px solid rgba(245,158,11,0.25); padding:0.4rem 0.6rem; border-radius:6px; font-size:0.78rem; color:#fef08a;">
            ✨ <strong>${i18n.t('honhach_breakthrough')}</strong> ${i18n.translateConcept(item.set4.extra24)}
          </div>
        ` : '';
        ...
        set4Html = `
          <div class="set-box" style="border-color: rgba(245,158,11,0.3);">
            <div class="set-box-title" style="color:var(--accent-gold);">
              <span>${i18n.t('honhach_set4_title')}</span>
      ...
      return `
        <div class="honhach-card">
          <div class="honhach-header">
            ...
            <div class="honhach-title-block">
              <div style="display:flex; justify-content:space-between; align-items:center;">
                <h3 class="honhach-name-vi">${nameDisplay}</h3>
          ...
          <p style="font-size:0.84rem; color:var(--text-sub); line-height:1.5;">${descDisplay}</p>
          ${set2Html}
          ${set4Html}
        </div>
      `;
  ...
  window.addEventListener('languageChanged', () => {
    if (searchInput) searchInput.placeholder = i18n.t('search_honhach_placeholder');
    filterAndRender();
  });
